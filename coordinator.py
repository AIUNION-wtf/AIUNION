...
def check_openrouter_balance(verbose: bool = True) -> float | None:
    """
    Fetch current OpenRouter credit balance via /api/v1/credits.
    Writes result to openrouter_status.json for the status dashboard.
    Returns the remaining balance in USD, or None on failure.
    """
    try:
        ...
        total_credits = float(data.get("data", {}).get("total_credits", 0) or 0)
        total_usage   = float(data.get("data", {}).get("total_usage",   0) or 0)
        ...
        if verbose:
            print(f"  U0001f4b3 OpenRouter balance: ${balance:.4f}  (${total_usage:.4f} used of ${total_credits:.4f} purchased)")
    ...
    for artifact in pending:
        ...
        if pro_bono is None or frozen_amount_usd is None:
            ...
            if _co_record is None:
                ...
                print(f"   [WARN] {claim_id}: no checkout record; assuming non-pro-bono, frozen_amount_usd={amount_usd}")
                pro_bono = False
